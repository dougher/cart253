...
class Ball {
  ...
  //Collide() is overloaded to allow collision between the ball
  //and the water drops so they affect its vertical movement (but
  //not horizontal to ensure the ball doesn't stay stuck by the 
  //water without allow the player(s) to do anything)
  void collide(Water _waters[]){
    for (int i=0; i<_waters.length; i++) {
      boolean insideLeft = (x + SIZE/2 > _waters[i].x - _waters[i].SIZE/2);
      boolean insideRight = (x - SIZE/2 < _waters[i].x + _waters[i].SIZE/2);
      boolean insideTop = (y + SIZE/2 > _waters[i].y - _waters[i].SIZE/2);
      boolean insideBottom = (y - SIZE/2 < _waters[i].y + _waters[i].SIZE/2);
